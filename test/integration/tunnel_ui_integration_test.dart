...
// Generate mocks
@GenerateMocks([SimpleTunnelClient, DesktopClientDetectionService, AuthService])
...
void main() {
  group('Tunnel UI Integration Tests', () {
    ...
    setUp(() {
      ...
      when(mockTunnelClient.config).thenReturn(
        TunnelConfig(
    ...
    Widget createTestApp({required Widget child}) {
      return MultiProvider(
        providers: [
          ...
          ChangeNotifierProvider<DesktopClientDetectionService>.value(
            value: mockClientDetection,
          ),
          ChangeNotifierProvider<AuthService>.value(value: mockAuthService),
        ],
        child: MaterialApp.router(routerConfig: router, title: 'Test App'),
      );
    }

    testWidgets(
      'tunnel status indicator shows correct status and opens management panel',
      (WidgetTester tester) async {
        ...
        await tester.pumpWidget(
          createTestApp(
            child: const Scaffold(body: AppHeader(title: 'Test')),
          ),
        );
    ...
    testWidgets('tunnel status updates consistently across components', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets(
      'tunnel wizard opens with correct mode from different entry points',
      (WidgetTester tester) async {
        await tester.pumpWidget(
          createTestApp(
            child: const Scaffold(body: AppHeader(title: 'Test')),
    ...
    testWidgets('desktop client prompt shows tunnel setup option', (
      WidgetTester tester,
    ) async {
      ...
      await tester.pumpWidget(
        createTestApp(child: const Scaffold(body: DesktopClientPrompt())),
      );
    ...
    testWidgets('settings screen shows enhanced tunnel status summary', (
      WidgetTester tester,
    ) async {
      ...
      await tester.pumpWidget(
        createTestApp(child: const Scaffold(body: UnifiedSettingsScreen())),
      );
      ...
      final foundStatus = statusTexts.any(
        (status) => find.text(status).evaluate().isNotEmpty,
      );
      expect(foundStatus, isTrue);
    });

    testWidgets('tunnel management panel provides comprehensive controls', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        createTestApp(
    ...
    testWidgets('tunnel status screen provides comprehensive monitoring', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        createTestApp(child: const Scaffold(body: TunnelStatusScreen())),
      );
    ...
    testWidgets('tunnel wizard supports multiple modes correctly', (
      WidgetTester tester,
    ) async {
      ...
      expect(
        find.text('Update your existing tunnel configuration'),
        findsOneWidget,
      );
      ...
      expect(
        find.text('Diagnose and fix tunnel connection issues'),
        findsOneWidget,
      );
    });

    testWidgets('error states are handled gracefully across components', (
      WidgetTester tester,
    ) async {
      ...
      expect(
        find.textContaining('Connection timeout'),
        findsAtLeastNWidgets(1),
      );
    });

    testWidgets('navigation between tunnel-related screens works correctly', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        createTestApp(
