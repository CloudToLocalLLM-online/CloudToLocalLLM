...
$deploymentCommand = "cd $VPSProjectPath && git reset --hard HEAD && git clean -fd && git pull origin master && /opt/flutter/bin/flutter clean && /opt/flutter/bin/flutter pub get && /opt/flutter/bin/flutter build web && docker compose restart webapp && sleep 10 && curl -k -f https://app.cloudtolocalllm.online"
