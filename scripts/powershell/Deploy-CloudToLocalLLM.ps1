...
$deploymentCommand = "cd $VPSProjectPath && sudo rm -rf certbot/ && git reset --hard HEAD && git clean -fd && git pull origin master && /opt/flutter/bin/flutter clean && /opt/flutter/bin/flutter pub get && /opt/flutter/bin/flutter build web && docker compose down && docker compose up -d && sleep 10 && curl -k -f https://app.cloudtolocalllm.online"
