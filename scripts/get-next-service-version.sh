...
if [ -z "$SERVICE_NAME" ] || [ -z "$ACR_NAME" ]; then
    echo "Usage: $0 <service-name> <acr-name>"
    echo "Example: $0 web imrightguycloudtolocalllm"
    exit 1
fi

echo "Fetching latest version for service: $SERVICE_NAME from ACR: $ACR_NAME"
...
if [ -z "$LATEST_TAG" ] || [ "$LATEST_TAG" = "null" ]; then
    ...
    echo "No existing version found, starting from: $BASE_VERSION"
    echo "$BASE_VERSION"
    exit 0
fi

echo "Latest existing version: $LATEST_TAG"
