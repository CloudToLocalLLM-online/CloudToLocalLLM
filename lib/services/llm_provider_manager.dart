...
class LLMProviderManager extends ChangeNotifier {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _logger.logTunnelError('PROVIDER_MANAGER_INIT_FAILED', _lastError!, error: e);
      rethrow;
    } finally {
      _setLoading(false);
    }
  ...
  Future<void> _loadPreferences() async {
    try {
      ...
      if (configsJson != null) {
        // TODO: Load and apply provider configurations
      }
    } catch (e) {
      _logger.logTunnelError('LOAD_PREFERENCES_FAILED', 'Failed to load preferences', error: e);
    }
  }
  
  /// Set active provider from preferences
  Future<void> _setActiveProviderFromPreferences() async {
    if (_activeProviderId != null && _providers.containsKey(_activeProviderId)) {
      try {
