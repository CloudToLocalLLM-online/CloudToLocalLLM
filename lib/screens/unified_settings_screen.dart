...
class _UnifiedSettingsScreenState extends State<UnifiedSettingsScreen> {
  ...
  Widget _buildUnifiedConnectionStatusCard(
    SimpleTunnelClient simpleTunnelClient,
  ) {
      ...
      builder: (context, localOllama, child) {
        return Consumer<AuthService>(
          builder: (context, authService, child) {
            ...
            final isConnecting = simpleTunnelClient.isConnecting;
    ...
    return ModernCard(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: AppTheme.spacingM),
          TextFormField(
            initialValue: tunnelClient.config.cloudProxyUrl,
            decoration: const InputDecoration(
            ...
            onChanged: (value) {
              _updateCloudConfig(tunnelClient, url: value);
            },
          ...
          SwitchListTile(
            ...
            onChanged: (value) {
              _updateCloudConfig(tunnelClient, enabled: value);
            },
            dense: true,
          ...
          SizedBox(
            width: double.infinity,
            child: ElevatedButton.icon(
              onPressed: !isProxyEnabled || tunnelClient.isConnecting
                  ? null
                  : () async {
                      await _refreshConnections(tunnelClient);
                    },
              icon: tunnelClient.isConnecting
                  ? const SizedBox(
                      width: 16,
                      height: 16,
                      child: CircularProgressIndicator(
  ...
  Widget _buildDesktopAdvancedTunnelCard(SimpleTunnelClient tunnelClient) {
        ...
        children: [
          ...
          _buildSettingItem(
            ...
            DropdownButton<int>(
              ...
              onChanged: (value) {
                if (value != null) {
                  _updateAdvancedConfig(tunnelClient, connectionTimeout: value);
                }
              },
            ),
          ),
          ...
          SizedBox(
            width: double.infinity,
            child: ElevatedButton.icon(
              onPressed: () => _resetToDefaults(tunnelClient),
  ...
  Future<void> _refreshConnections(SimpleTunnelClient tunnelClient) async {
    try {
      // Get local Ollama service before async operations
      final localOllama = context.read<LocalOllamaConnectionService>();

      // Refresh tunnel client connections (cloud proxy only)
      await tunnelClient.initialize();
  ...
  Future<void> _resetToDefaults(SimpleTunnelClient tunnelClient) async {
    try {
      // Default config is created but not used directly as SimpleTunnelClient handles updates
      // Keep the variable for future implementation
