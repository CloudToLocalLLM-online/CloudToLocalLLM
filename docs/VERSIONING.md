...
- **Web**: `4.4.3`
- **API**: `4.4.3-api`
- **Streaming Proxy**: `4.4.3-proxy`
- **Postgres**: `4.4.3-postgres`
- **Base**: `4.4.3-base`
...
- **Patch** (4.4.3 → 4.4.3): Bug fixes, minor changes
- **Minor** (4.4.3 → 4.4.3): New features, backwards compatible
- **Major** (4.4.3 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.4.3-api`)
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.4.3
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.4.3 -n cloudtolocalllm
```
...
Build Process:
├─ Web: SKIP (no changes) → use 4.4.3 (latest)
├─ API: BUILD → tag as 4.4.3-api
...
Versioning:
└─ Bump to: 4.4.3 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.4.3, 4.4.3-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.4.3 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
