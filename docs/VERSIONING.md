...
- **Web**: `4.6.7`
- **API**: `4.6.7-api`
- **Streaming Proxy**: `4.6.7-proxy`
- **Postgres**: `4.6.7-postgres`
- **Base**: `4.6.7-base`
...
- **Patch** (4.6.7 → 4.6.7): Bug fixes, minor changes
- **Minor** (4.6.7 → 4.6.7): New features, backwards compatible
- **Major** (4.6.7 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.6.7-api`)
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.6.7
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.6.7 -n cloudtolocalllm
```
...
Build Process:
├─ Web: SKIP (no changes) → use 4.6.7 (latest)
├─ API: BUILD → tag as 4.6.7-api
...
Versioning:
└─ Bump to: 4.6.7 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.6.7, 4.6.7-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.6.7 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
