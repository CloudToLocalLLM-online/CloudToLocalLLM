...
- **Web**: `4.6.17`
- **API**: `4.6.17-api`
- **Streaming Proxy**: `4.6.17-proxy`
- **Postgres**: `4.6.17-postgres`
- **Base**: `4.6.17-base`
...
- **Patch** (4.6.17 → 4.6.17): Bug fixes, minor changes
- **Minor** (4.6.17 → 4.6.17): New features, backwards compatible
- **Major** (4.6.17 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.6.17-api`)
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.6.17
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.6.17 -n cloudtolocalllm
```
...
Build Process:
├─ Web: SKIP (no changes) → use 4.6.17 (latest)
├─ API: BUILD → tag as 4.6.17-api
...
Versioning:
└─ Bump to: 4.6.17 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.6.17, 4.6.17-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.6.17 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
