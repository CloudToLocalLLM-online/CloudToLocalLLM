...
- **Web**: `4.14.1`
- **API**: `4.14.1-api`
- **Streaming Proxy**: `4.14.1-proxy`
- **Postgres**: `4.14.1-postgres`
- **Base**: `4.14.1-base`
...
- **Patch** (4.14.1 → 4.14.1): Bug fixes, minor changes
- **Minor** (4.14.1 → 4.14.1): New features, backwards compatible
- **Major** (4.14.1 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.14.1-api`)
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.14.1
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.14.1 -n cloudtolocalllm
```
...
Build Process:
├─ Web: SKIP (no changes) → use 4.14.1 (latest)
├─ API: BUILD → tag as 4.14.1-api
...
Versioning:
└─ Bump to: 4.14.1 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.14.1, 4.14.1-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.14.1 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
