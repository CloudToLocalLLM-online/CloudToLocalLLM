...
- **Web**: `4.4.5`
- **API**: `4.4.5-api`
- **Streaming Proxy**: `4.4.5-proxy`
- **Postgres**: `4.4.5-postgres`
- **Base**: `4.4.5-base`
...
- **Patch** (4.4.5 → 4.4.5): Bug fixes, minor changes
- **Minor** (4.4.5 → 4.4.5): New features, backwards compatible
- **Major** (4.4.5 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.4.5-api`)
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.4.5
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.4.5 -n cloudtolocalllm
```
...
Build Process:
├─ Web: SKIP (no changes) → use 4.4.5 (latest)
├─ API: BUILD → tag as 4.4.5-api
...
Versioning:
└─ Bump to: 4.4.5 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.4.5, 4.4.5-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.4.5 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
