...
- **Patch** (4.5.0 → 4.5.1): Bug fixes, minor changes
- **Minor** (4.5.0 → 4.6.0): New features, backwards compatible
- **Major** (4.5.0 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.5.0-api`)
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.5.0
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.4.0 -n cloudtolocalllm
```
...
Build Process:
├─ Web: SKIP (no changes) → use 4.5.0 (latest)
├─ API: BUILD → tag as 4.5.0-api
