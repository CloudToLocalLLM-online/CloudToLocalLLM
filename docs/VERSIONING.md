...
- **Patch** (4.5.0 → 4.5.0): Bug fixes, minor changes
- **Minor** (4.5.0 → 4.5.0): New features, backwards compatible
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.5.0 -n cloudtolocalllm
```
...
Versioning:
└─ Bump to: 4.5.0 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.5.0, 4.5.0-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.5.0 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
