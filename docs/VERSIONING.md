...
- **Web**: `4.12.17`
- **API**: `4.12.17-api`
- **Streaming Proxy**: `4.12.17-proxy`
- **Postgres**: `4.12.17-postgres`
- **Base**: `4.12.17-base`
...
- **Patch** (4.12.17 → 4.12.17): Bug fixes, minor changes
- **Minor** (4.12.17 → 4.12.17): New features, backwards compatible
- **Major** (4.12.17 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.12.17-api`)
...
metadata:
  annotations:
    kubernetes.io/revision: "4.12.17"
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.12.17
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.12.17 -n cloudtolocalllm
```
...
Current version: 4.12.17

Build Process:
├─ Web: SKIP (no changes) → use 4.12.17 (latest)
├─ API: BUILD → tag as 4.12.17-api
...
Versioning:
└─ Bump to: 4.12.17 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.12.17, 4.12.17-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.12.17 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
