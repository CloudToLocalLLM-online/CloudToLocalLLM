...
- **Web**: `4.20.3`
- **API**: `4.20.3-api`
- **Streaming Proxy**: `4.20.3-proxy`
- **Postgres**: `4.20.3-postgres`
- **Base**: `4.20.3-base`
...
- **Patch** (4.20.3 → 4.20.3): Bug fixes, minor changes
- **Minor** (4.20.3 → 4.20.3): New features, backwards compatible
- **Major** (4.20.3 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.20.3-api`)
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.20.3
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.20.3 -n cloudtolocalllm
```
...
Build Process:
├─ Web: SKIP (no changes) → use 4.20.3 (latest)
├─ API: BUILD → tag as 4.20.3-api
...
Versioning:
└─ Bump to: 4.20.3 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.20.3, 4.20.3-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.20.3 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
