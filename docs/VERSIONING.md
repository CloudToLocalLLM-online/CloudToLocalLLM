...
- **Web**: `4.6.20`
- **API**: `4.6.20-api`
- **Streaming Proxy**: `4.6.20-proxy`
- **Postgres**: `4.6.20-postgres`
- **Base**: `4.6.20-base`
...
- **Patch** (4.6.20 → 4.6.20): Bug fixes, minor changes
- **Minor** (4.6.20 → 4.6.20): New features, backwards compatible
- **Major** (4.6.20 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.6.20-api`)
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.6.20
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.6.20 -n cloudtolocalllm
```
...
Current version: 4.6.20

Build Process:
├─ Web: SKIP (no changes) → use 4.6.20 (latest)
├─ API: BUILD → tag as 4.6.20-api
...
Versioning:
└─ Bump to: 4.6.20 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.6.20, 4.6.20-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.6.20 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
