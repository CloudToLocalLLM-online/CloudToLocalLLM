...
- **Web**: `4.9.2`
- **API**: `4.9.2-api`
- **Streaming Proxy**: `4.9.2-proxy`
- **Postgres**: `4.9.2-postgres`
- **Base**: `4.9.2-base`
...
- **Patch** (4.9.2 → 4.9.2): Bug fixes, minor changes
- **Minor** (4.9.2 → 4.9.2): New features, backwards compatible
- **Major** (4.9.2 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.9.2-api`)
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.9.2
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.9.2 -n cloudtolocalllm
```
...
Build Process:
├─ Web: SKIP (no changes) → use 4.9.2 (latest)
├─ API: BUILD → tag as 4.9.2-api
...
Versioning:
└─ Bump to: 4.9.2 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.9.2, 4.9.2-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.9.2 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
