...
- **Web**: `4.4.6`
- **API**: `4.4.6-api`
- **Streaming Proxy**: `4.4.6-proxy`
- **Postgres**: `4.4.6-postgres`
- **Base**: `4.4.6-base`
...
- **Patch** (4.4.6 → 4.4.6): Bug fixes, minor changes
- **Minor** (4.4.6 → 4.4.6): New features, backwards compatible
- **Major** (4.4.6 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.4.6-api`)
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.4.6
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.4.6 -n cloudtolocalllm
```
...
Build Process:
├─ Web: SKIP (no changes) → use 4.4.6 (latest)
├─ API: BUILD → tag as 4.4.6-api
...
Versioning:
└─ Bump to: 4.4.6 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.4.6, 4.4.6-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.4.6 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
