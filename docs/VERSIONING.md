...
- **Web**: `4.4.10`
- **API**: `4.4.10-api`
- **Streaming Proxy**: `4.4.10-proxy`
- **Postgres**: `4.4.10-postgres`
- **Base**: `4.4.10-base`
...
- **Patch** (4.4.10 → 4.4.10): Bug fixes, minor changes
- **Minor** (4.4.10 → 4.4.10): New features, backwards compatible
- **Major** (4.4.10 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.4.10-api`)
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.4.10
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.4.10 -n cloudtolocalllm
```
...
Build Process:
├─ Web: SKIP (no changes) → use 4.4.10 (latest)
├─ API: BUILD → tag as 4.4.10-api
...
Versioning:
└─ Bump to: 4.4.10 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.4.10, 4.4.10-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.4.10 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
