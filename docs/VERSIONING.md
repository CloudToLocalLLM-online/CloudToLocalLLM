...
- **Web**: `4.12.16`
- **API**: `4.12.16-api`
- **Streaming Proxy**: `4.12.16-proxy`
- **Postgres**: `4.12.16-postgres`
- **Base**: `4.12.16-base`
...
- **Patch** (4.12.16 → 4.12.16): Bug fixes, minor changes
- **Minor** (4.12.16 → 4.12.16): New features, backwards compatible
- **Major** (4.12.16 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.12.16-api`)
...
metadata:
  annotations:
    kubernetes.io/revision: "4.12.16"
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.12.16
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.12.16 -n cloudtolocalllm
```
...
Current version: 4.12.16

Build Process:
├─ Web: SKIP (no changes) → use 4.12.16 (latest)
├─ API: BUILD → tag as 4.12.16-api
...
Versioning:
└─ Bump to: 4.12.16 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.12.16, 4.12.16-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.12.16 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
