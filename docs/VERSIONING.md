...
- **Web**: `4.20.27`
- **API**: `4.20.27-api`
- **Streaming Proxy**: `4.20.27-proxy`
- **Postgres**: `4.20.27-postgres`
- **Base**: `4.20.27-base`
...
- **Patch** (4.20.27 → 4.20.27): Bug fixes, minor changes
- **Minor** (4.20.27 → 4.20.27): New features, backwards compatible
- **Major** (4.20.27 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.20.27-api`)
...
metadata:
  annotations:
    kubernetes.io/revision: "4.20.27"
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.20.27
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.20.27 -n cloudtolocalllm
```
...
Current version: 4.20.27

Build Process:
├─ Web: SKIP (no changes) → use 4.20.27 (latest)
├─ API: BUILD → tag as 4.20.27-api
...
Versioning:
└─ Bump to: 4.20.27 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.20.27, 4.20.27-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.20.27 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
