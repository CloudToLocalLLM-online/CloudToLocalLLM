...
- **Web**: `4.14.4`
- **API**: `4.14.4-api`
- **Streaming Proxy**: `4.14.4-proxy`
- **Postgres**: `4.14.4-postgres`
- **Base**: `4.14.4-base`
...
- **Patch** (4.14.4 → 4.14.4): Bug fixes, minor changes
- **Minor** (4.14.4 → 4.14.4): New features, backwards compatible
- **Major** (4.14.4 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.14.4-api`)
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.14.4
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.14.4 -n cloudtolocalllm
```
...
Build Process:
├─ Web: SKIP (no changes) → use 4.14.4 (latest)
├─ API: BUILD → tag as 4.14.4-api
...
Versioning:
└─ Bump to: 4.14.4 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.14.4, 4.14.4-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.14.4 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
