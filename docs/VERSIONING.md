...
- **Web**: `4.12.0`
- **API**: `4.12.0-api`
- **Streaming Proxy**: `4.12.0-proxy`
- **Postgres**: `4.12.0-postgres`
- **Base**: `4.12.0-base`
...
- **Patch** (4.12.0 → 4.12.0): Bug fixes, minor changes
- **Minor** (4.12.0 → 4.12.0): New features, backwards compatible
- **Major** (4.12.0 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.12.0-api`)
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.12.0
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.12.0 -n cloudtolocalllm
```
...
Build Process:
├─ Web: SKIP (no changes) → use 4.12.0 (latest)
├─ API: BUILD → tag as 4.12.0-api
...
Versioning:
└─ Bump to: 4.12.0 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.12.0, 4.12.0-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.12.0 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
