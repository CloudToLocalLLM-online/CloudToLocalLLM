...
- **Web**: `4.20.25`
- **API**: `4.20.25-api`
- **Streaming Proxy**: `4.20.25-proxy`
- **Postgres**: `4.20.25-postgres`
- **Base**: `4.20.25-base`
...
- **Patch** (4.20.25 → 4.20.25): Bug fixes, minor changes
- **Minor** (4.20.25 → 4.20.25): New features, backwards compatible
- **Major** (4.20.25 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.20.25-api`)
...
metadata:
  annotations:
    kubernetes.io/revision: "4.20.25"
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.20.25
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.20.25 -n cloudtolocalllm
```
...
Current version: 4.20.25

Build Process:
├─ Web: SKIP (no changes) → use 4.20.25 (latest)
├─ API: BUILD → tag as 4.20.25-api
...
Versioning:
└─ Bump to: 4.20.25 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.20.25, 4.20.25-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.20.25 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
