...
- **Web**: `4.20.8`
- **API**: `4.20.8-api`
- **Streaming Proxy**: `4.20.8-proxy`
- **Postgres**: `4.20.8-postgres`
- **Base**: `4.20.8-base`
...
- **Patch** (4.20.8 → 4.20.8): Bug fixes, minor changes
- **Minor** (4.20.8 → 4.20.8): New features, backwards compatible
- **Major** (4.20.8 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.20.8-api`)
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.20.8
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.20.8 -n cloudtolocalllm
```
...
Build Process:
├─ Web: SKIP (no changes) → use 4.20.8 (latest)
├─ API: BUILD → tag as 4.20.8-api
...
Versioning:
└─ Bump to: 4.20.8 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.20.8, 4.20.8-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.20.8 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
