...
- **Web**: `4.12.13`
- **API**: `4.12.13-api`
- **Streaming Proxy**: `4.12.13-proxy`
- **Postgres**: `4.12.13-postgres`
- **Base**: `4.12.13-base`
...
- **Patch** (4.12.13 → 4.12.13): Bug fixes, minor changes
- **Minor** (4.12.13 → 4.12.13): New features, backwards compatible
- **Major** (4.12.13 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.12.13-api`)
...
metadata:
  annotations:
    kubernetes.io/revision: "4.12.13"
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.12.13
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.12.13 -n cloudtolocalllm
```
...
Current version: 4.12.13

Build Process:
├─ Web: SKIP (no changes) → use 4.12.13 (latest)
├─ API: BUILD → tag as 4.12.13-api
...
Versioning:
└─ Bump to: 4.12.13 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.12.13, 4.12.13-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.12.13 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
