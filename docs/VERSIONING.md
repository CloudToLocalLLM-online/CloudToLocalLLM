...
- **Web**: `4.6.19`
- **API**: `4.6.19-api`
- **Streaming Proxy**: `4.6.19-proxy`
- **Postgres**: `4.6.19-postgres`
- **Base**: `4.6.19-base`
...
- **Patch** (4.6.19 → 4.6.19): Bug fixes, minor changes
- **Minor** (4.6.19 → 4.6.19): New features, backwards compatible
- **Major** (4.6.19 → 5.0.0): Breaking changes
...
- ✅ **Uses semantic version tag** (e.g., `4.6.19-api`)
...
```
imrightguycloudtolocalllm.azurecr.io/web:4.6.19
...
kubectl set image deployment/web web=imrightguycloudtolocalllm.azurecr.io/web:4.6.19 -n cloudtolocalllm
```
...
Current version: 4.6.19

Build Process:
├─ Web: SKIP (no changes) → use 4.6.19 (latest)
├─ API: BUILD → tag as 4.6.19-api
...
Versioning:
└─ Bump to: 4.6.19 (patch bump)

Build Process:
├─ Web: BUILD → tag as 4.6.19, 4.6.19-api, etc.
├─ API: SKIP → use latest
├─ Proxy: SKIP → use latest
└─ Postgres: SKIP → use latest

Deployment:
├─ Web: 4.6.19 (new version!)
├─ API: latest (cached)
├─ Proxy: latest (cached)
└─ Postgres: latest (cached)
```
