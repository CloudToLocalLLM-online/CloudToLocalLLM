...
### **Simplified Tunnel System (In Development)**
CloudToLocalLLM is implementing a simplified tunnel system to replace the current complex multi-layered architecture. This new system reduces codebase complexity by ~70% while maintaining security and reliability.

For complete technical details, see [Simplified Tunnel Architecture](SIMPLIFIED_TUNNEL_ARCHITECTURE.md).

**Key Features:**
- **Message Protocol**: Standardized communication protocol implemented in `api-backend/tunnel/message-protocol.js`
- **Single WebSocket**: One persistent connection per desktop client
- **HTTP Proxy Pattern**: Standard HTTP requests to proxy endpoints
- **JWT Authentication**: Token-based user identification and security
