...
```javascript
import { adminBurstLimiter, combineRateLimiters } from '../middleware/admin-rate-limiter.js';
router.post('/critical', combineRateLimiters(adminBurstLimiter, adminRateLimiter), handler);
```
...
router.get('/reports/export', adminExpensiveLimiter, adminAuth(['export_reports']), handler);
```
...
router.post('/flush/execute', adminCriticalLimiter, adminAuth(['super_admin']), handler);
```
