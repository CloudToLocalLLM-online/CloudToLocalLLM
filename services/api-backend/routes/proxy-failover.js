...
export function createProxyFailoverRoutes(db, logger) {
  ...
  router.post('/proxy/failover/config', authenticateJWT, async (req, res) => {
    try {
      ...
      const result = await failoverService.createFailoverConfiguration(proxyId, userId, config);
  ...
  router.post('/proxy/instances', authenticateJWT, async (req, res) => {
    try {
      ...
      const result = await failoverService.registerProxyInstance(proxyId, userId, instanceData);
  ...
  router.put('/proxy/instances/:instanceId/health', authenticateJWT, async (req, res) => {
    try {
      ...
      const result = await failoverService.updateInstanceHealth(instanceId, healthStatus, metrics);
      ...
      if (!proxyId || !sourceInstanceId || !targetInstanceId) {
        return res.status(400).json({
          error: {
            ...
            message: 'proxyId, sourceInstanceId, and targetInstanceId are required',
            statusCode: 400,
          },
        });
      }
  ...
  router.put('/proxy/failover/events/:eventId/complete', authenticateJWT, authorizeRBAC('admin'), async (req, res) => {
    try {
      ...
      const result = await failoverService.completeFailoverEvent(eventId, status, errorMessage, durationMs);
  ...
  router.put('/proxy/:proxyId/redundancy', authenticateJWT, authorizeRBAC('admin'), async (req, res) => {
    try {
      ...
      const result = await failoverService.updateRedundancyStatus(proxyId, userId, statusData);
