...
router.post('/scaling/policies/:proxyId', authenticateJWT, addTierInfo, async (req, res) => {
  try {
    ...
    try {
      const createdPolicy = await proxyScalingService.createScalingPolicy(proxyId, userId, policy);
...
router.post('/scaling/metrics/:proxyId', authenticateJWT, addTierInfo, async (req, res) => {
  try {
    ...
    try {
      const recordedMetrics = await proxyScalingService.recordLoadMetrics(proxyId, userId, metrics);
...
router.post('/scaling/evaluate/:proxyId', authenticateJWT, addTierInfo, async (req, res) => {
  try {
    ...
    const decision = await proxyScalingService.evaluateScaling(proxyId, userId);
...
router.get('/scaling/summary/:proxyId', authenticateJWT, addTierInfo, async (req, res) => {
  try {
    ...
    const summary = await proxyScalingService.getScalingSummary(proxyId, hoursBack);
