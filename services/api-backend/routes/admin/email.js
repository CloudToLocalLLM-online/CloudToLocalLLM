...
function initializeServices(pool) {
  ...
  if (!emailQueueService) {
    emailQueueService = new EmailQueueService(pool, googleWorkspaceService, emailConfigService);
  }
}
 ...
 */
router.post('/oauth/start', adminWriteLimiter, adminAuth(['manage_email_config']), async (req, res) => {
  try {
    const pool = getPool();
 ...
 */
router.post('/oauth/callback', adminWriteLimiter, adminAuth(['manage_email_config']), async (req, res) => {
  try {
    const pool = getPool();
    initializeServices(pool);
 ...
 */
router.get('/config', adminReadOnlyLimiter, adminAuth(['view_email_config']), async (req, res) => {
  try {
    const pool = getPool();
    initializeServices(pool);

    const configs = await emailConfigService.getAllConfigurations(req.adminUser.id);
 ...
 */
router.delete('/config', adminWriteLimiter, adminAuth(['manage_email_config']), async (req, res) => {
  try {
    const pool = getPool();
    initializeServices(pool);
 ...
 * Send test email
 */
router.post('/test', adminWriteLimiter, adminAuth(['manage_email_config']), async (req, res) => {
  try {
    const pool = getPool();
    initializeServices(pool);

    const { recipientEmail, subject = 'Test Email from CloudToLocalLLM' } = req.body;
    ...
    const config = await googleWorkspaceService.getOAuthConfiguration(req.adminUser.id);

    if (!config) {
      return res.status(400).json({
        error: 'No Google Workspace configuration found. Please set up OAuth first.',
        code: 'NO_CONFIG'
      });
    }
 ...
 */
router.get('/status', adminReadOnlyLimiter, adminAuth(['view_email_config']), async (req, res) => {
  try {
    const pool = getPool();
    initializeServices(pool);
 ...
 */
router.get('/quota', adminReadOnlyLimiter, adminAuth(['view_email_config']), async (req, res) => {
  try {
    const pool = getPool();
    initializeServices(pool);
    ...
    const quotaData = await googleWorkspaceService.getQuotaUsage(req.adminUser.id);
 ...
 */
router.get('/templates', adminReadOnlyLimiter, adminAuth(['view_email_config']), async (req, res) => {
  try {
    const pool = getPool();
    initializeServices(pool);
    ...
    const templates = await emailConfigService.listTemplates(req.adminUser.id, {
      limit,
      offset
    });
 ...
 */
router.post('/templates', adminWriteLimiter, adminAuth(['manage_email_config']), async (req, res) => {
  try {
    const pool = getPool();
    initializeServices(pool);
 ...
 */
router.put('/templates/:id', adminWriteLimiter, adminAuth(['manage_email_config']), async (req, res) => {
  try {
    const pool = getPool();
    initializeServices(pool);
    ...
    `;
    const getResult = await pool.query(getQuery, [templateId, req.adminUser.id]);
    ...
    const updateName = name !== undefined ? name.trim() : existingTemplate.name;
    const updateSubject = subject !== undefined ? subject.trim() : existingTemplate.subject;
    const updateHtmlBody = html_body !== undefined ? html_body.trim() : existingTemplate.html_body;
    const updateTextBody = text_body !== undefined ? (text_body ? text_body.trim() : null) : existingTemplate.text_body;
    const updateDescription = description !== undefined ? (description ? description.trim() : null) : existingTemplate.description;
    const updateVariables = variables !== undefined ? variables : (typeof existingTemplate.variables === 'string' ? JSON.parse(existingTemplate.variables) : existingTemplate.variables);
 ...
 */
router.delete('/templates/:id', adminWriteLimiter, adminAuth(['manage_email_config']), async (req, res) => {
  try {
    const pool = getPool();
    initializeServices(pool);
 ...
 * - Failure reasons
 */
router.get('/metrics', adminReadOnlyLimiter, adminAuth(['view_email_config']), async (req, res) => {
  try {
    const pool = getPool();
    initializeServices(pool);
    ...
    const failureReasonsResult = await pool.query(failureReasonsQuery, [startDate, endDate]);
      ...
      data: {
        metrics: {
          summary: {
            ...
            successRate: metrics.total_count > 0 ? ((metrics.sent_count / metrics.total_count) * 100).toFixed(2) : 0
          },
          deliveryTime: {
            average: metrics.avg_delivery_time_seconds ? parseFloat(metrics.avg_delivery_time_seconds).toFixed(2) : null,
            min: metrics.min_delivery_time_seconds ? parseFloat(metrics.min_delivery_time_seconds).toFixed(2) : null,
            max: metrics.max_delivery_time_seconds ? parseFloat(metrics.max_delivery_time_seconds).toFixed(2) : null,
            p50: metrics.p50_delivery_time_seconds ? parseFloat(metrics.p50_delivery_time_seconds).toFixed(2) : null,
            p95: metrics.p95_delivery_time_seconds ? parseFloat(metrics.p95_delivery_time_seconds).toFixed(2) : null,
            p99: metrics.p99_delivery_time_seconds ? parseFloat(metrics.p99_delivery_time_seconds).toFixed(2) : null
          },
 ...
 * - Total count
 */
router.get('/delivery-logs', adminReadOnlyLimiter, adminAuth(['view_email_config']), async (req, res) => {
  try {
    const pool = getPool();
    initializeServices(pool);
    ...
    if (!validStatuses.includes(status)) {
      return res.status(400).json({
        error: 'Invalid status filter. Valid values: sent, failed, bounced, pending, all',
        code: 'INVALID_STATUS'
      });
    }

    // Validate sort parameters
    const sortBy = ['created_at', 'sent_at', 'status'].includes(req.query.sortBy) ? req.query.sortBy : 'created_at';
    const sortOrder = ['asc', 'desc'].includes(req.query.sortOrder) ? req.query.sortOrder.toUpperCase() : 'DESC';
