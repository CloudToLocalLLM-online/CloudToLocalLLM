 ...
 */
router.get('/revenue', adminReadOnlyLimiter, adminAuth(['view_reports']), async (req, res) => {
  try {
    const pool = getPool();
    ...
    const response = {
      ...
      averageTransactionValue: parseFloat(overallMetrics.average_transaction_value)
    };
 ...
 * - period: Date range of the report
 */
router.get('/subscriptions', adminReadOnlyLimiter, adminAuth(['view_reports']), async (req, res) => {
  try {
    const pool = getPool();
    ...
    const canceledQuery = `
      ...
      WHERE canceled_at >= $1
        AND canceled_at <= $2
    `;
    const canceledResult = await pool.query(canceledQuery, [startDate, endDate]);
    ...
    const response = {
      ...
      metrics: {
        ...
        subscriptionsAtPeriodEnd: subscriptionsAtStart + newSubscriptions - canceledSubscriptions,
    ...
    logger.info('[AdminReports] Subscription metrics report generated successfully', {
      adminUserId: req.adminUser.id,
      activeSubscriptions,
      churnRate: response.churnRate,
      mrr: monthlyRecurringRevenue
    ...
    logger.error('[AdminReports] Error generating subscription metrics report', {
      error: error.message,
      stack: error.stack,
      adminUserId: req.adminUser?.id
    });
 ...
 */
router.get('/export', adminExpensiveLimiter, adminAuth(['export_reports']), async (req, res) => {
  try {
    const pool = getPool();
    ...
    switch (type) {
      case 'revenue':
        ...
        headers = ['id', 'created_at', 'user_email', 'username', 'amount', 'currency', 'status', 'subscription_tier', 'payment_method_type', 'payment_method_last4'];
        filename = `revenue_report_${startDate}_${endDate}`;
        break;

      case 'subscriptions':
        reportData = await generateSubscriptionReportData(pool, start, end);
        headers = ['id', 'created_at', 'user_email', 'username', 'tier', 'status', 'current_period_start', 'current_period_end', 'canceled_at', 'cancel_at_period_end'];
        filename = `subscription_report_${startDate}_${endDate}`;
        break;

      case 'transactions':
        reportData = await generateTransactionReportData(pool, start, end);
        headers = ['id', 'created_at', 'user_email', 'username', 'amount', 'currency', 'status', 'payment_method_type', 'payment_method_last4', 'stripe_payment_intent_id', 'subscription_tier'];
    ...
    if (format === 'csv') {
      ...
      res.setHeader('Content-Type', 'text/csv');
      res.setHeader('Content-Disposition', `attachment; filename="${filename}.csv"`);
      res.setHeader('X-PDF-Note', 'PDF export not yet implemented, returning CSV format');
      res.send(csv);
    }
