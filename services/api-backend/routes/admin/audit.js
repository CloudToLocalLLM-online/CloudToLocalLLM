 ...
 */
router.get('/logs', adminReadOnlyLimiter, adminAuth(['view_audit_logs']), async (req, res) => {
  try {
    const pool = getPool();
    ...
    // Validate sort field
    const validSortFields = ['created_at', 'action', 'resource_type', 'admin_user_id'];
 ...
 */
router.get('/logs/:logId', adminReadOnlyLimiter, adminAuth(['view_audit_logs']), async (req, res) => {
  try {
    const pool = getPool();
 ...
 */
router.get('/export', adminExpensiveLimiter, adminAuth(['export_audit_logs']), async (req, res) => {
  try {
    const pool = getPool();
