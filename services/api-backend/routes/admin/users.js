...
import { logAdminAction, auditMiddleware, createUserManagementDetails } from '../../utils/audit-logger.js';
 ...
 */
router.get('/', adminReadOnlyLimiter, adminAuth(['view_users']), async (req, res) => {
  try {
    const pool = getPool();
 ...
 * - Activity timeline
 */
router.get('/:userId', adminReadOnlyLimiter, adminAuth(['view_users']), async (req, res) => {
  try {
    const pool = getPool();
    ...
    const paymentHistoryResult = await pool.query(paymentHistoryQuery, [userId]);
 ...
 */
router.patch('/:userId', adminRateLimiter, adminAuth(['edit_users']), async (req, res) => {
  try {
    const pool = getPool();
    const { userId } = req.params;
    ...
    if (tierPricing[newTier] > tierPricing[previousTier]) {
      ...
      if (user.subscription_id) {
        const subscriptionDetailsQuery = `
          ...
          WHERE id = $1
        `;
        const subDetails = await pool.query(subscriptionDetailsQuery, [user.subscription_id]);
        
        if (subDetails.rows.length > 0) {
          const periodStart = new Date(subDetails.rows[0].current_period_start);
    ...
    try {
      ...
      if (user.subscription_id) {
        ...
        const updateQuery = `
          ...
          RETURNING id
        `;
        const updateResult = await pool.query(updateQuery, [newTier, user.subscription_id]);
 ...
 * - Logs action in audit log
 */
router.post('/:userId/suspend', adminRateLimiter, adminAuth(['suspend_users']), async (req, res) => {
  try {
    const pool = getPool();
    const { userId } = req.params;
    ...
    try {
      ...
      const sessionsResult = await pool.query(invalidateSessionsQuery, [userId]);
 ...
 * - Logs action in audit log
 */
router.post('/:userId/reactivate', adminRateLimiter, adminAuth(['suspend_users']), async (req, res) => {
  try {
    const pool = getPool();
    const { userId } = req.params;
    const { note } = req.body;
