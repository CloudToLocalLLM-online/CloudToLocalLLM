...
export async function initializeWebhookEventFilterService() {
  try {
    ...
    logger.info('[WebhookEventFilterRoutes] Webhook event filter service initialized');
  } catch (error) {
    logger.error('[WebhookEventFilterRoutes] Failed to initialize filter service', {
      error: error.message,
    });
    throw error;
  }
...
router.post('/:tunnelId/webhooks/:webhookId/filters', authenticateJWT, async (req, res) => {
  try {
    ...
    const filter = await filterService.createFilter(webhookId, userId, filterConfig);
...
router.put('/:tunnelId/webhooks/:webhookId/filters', authenticateJWT, async (req, res) => {
  try {
    ...
    const filter = await filterService.updateFilter(webhookId, userId, filterConfig);
