...
- **Web Application**: Flutter web app hosted on AWS EKS (migrating from Azure)
...
- **Infrastructure**: AWS EKS with CloudFormation, OIDC authentication
- **Monitoring**: Sentry error tracking, Grafana observability, cost monitoring

## Authentication & Security

- Auth0 OAuth2 authentication with JWT tokens (auth provider agnostic)
- Encrypted token storage using flutter_secure_storage
- User tier system (free, premium, enterprise)
- End-to-end encryption for all communications
- AWS OIDC authentication for CI/CD (no long-lived credentials)
